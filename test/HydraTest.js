...
(function (win, doc, Hydra) {
  ...
  oTestCase('HydraSubscribersTest', sinon.testCase({
    ...
    'test should check that must return an array with an element if a subscriber is registered': function () {
      ...
      Hydra.bus.subscribe(this.oSubscriber);
  ...
  oTestCase('HydraBusSubscribeTest', sinon.testCase({
    ...
    'test should check that no subscriber must be added if Subscriber does not have events and must return false': function () {
      var bResult = true;

      bResult = Hydra.bus.subscribe(this.oBadSubscriber);
    ...
    'test should check that one subscriber has been added to channel and other to global if Suscriber has events': function () {
      ...
      bResult = Hydra.bus.subscribe(this.oSubscriber);
      ...
      assertTrue(bResult);
      assertEquals(1, aChannelSubscribers.length);
      assertEquals(1, aGlobalSubscribers.length);
      assertSame(this.oSubscriber, aChannelSubscribers[0].subscriber);
      assertSame(this.oSubscriber, aGlobalSubscribers[0].subscriber);
      assertSame(this.oSubscriber.events.channel['item:actionChannel'], aChannelSubscribers[0].handler);
      assertSame(this.oSubscriber.events.global['item:actionGlobal'], aGlobalSubscribers[0].handler);

      Hydra.bus.unsubscribe(this.oSubscriber);
    }
  }));
  ...
  oTestCase('HydraBusUnsubscribeTest', sinon.testCase({
    ...
    'test should check that must return false if Subscriber does not have events': function () {
      var bResult = true;

      bResult = Hydra.bus.unsubscribe(this.oBadSubscriber);

      assertFalse(bResult);
    },
    'test should check that must return false if Subscriber has events but has not been subscribed': function () {
      var bResult = true;

      bResult = Hydra.bus.unsubscribe(this.oSubscriber);

      assertFalse(bResult);
    },
    'test should check that must return true if Subscriber has events but has been subscribed': function () {
      var bResult = false;
      Hydra.bus.subscribe(this.oSubscriber);
    ...
    'test should check that subscribers of global must have subscriber if unsubscribe is launched': function () {
  ...
  oTestCase('HydraBusPublishTest', sinon.testCase({
    ...
    'test should check that must return false if there are no subscribers to the event in channel': function () {
      ...
      assertFalse(bResult);
      assertEquals(0, this.oSubscriber.events.channel['item:action'].callCount);
    },
    'test should check that must return true if there are any subscriber to the event in channel': function () {
      ...
      assertTrue(bResult);
      assertEquals(1, this.oSubscriber.events.channel['item:action'].callCount);
    }
  }));
  oTestCase('HydraAddExtensionBeforeInitTest', sinon.testCase({
    setUp: function () {
      Hydra.module.oModifyInit = {};
    },
    tearDown: function () {
      Hydra.module.oModifyInit = {};
    },
    'test should check that Hydra.module.oModifyInit is an empty object': function () {
      assertEquals(0, getLengthObject(Hydra.module.oModifyInit));
    },
    'test should check that after using addExtensionBeforeInitTest it saves the object': function(){
      var stub = sinon.stub();
      Hydra.addExtensionBeforeInit({ test: stub});
      assertEquals(1, getLengthObject(Hydra.module.oModifyInit));
      assertSame(stub, Hydra.module.oModifyInit.test);
      Hydra.module.oModifyInit.test();
